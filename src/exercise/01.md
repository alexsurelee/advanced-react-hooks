...
The `useReducer` syntax is a touch confusing to follow.
...
Would provide you with a current `state` object that is updated upon calls to
`countReducer`.

So if you've got a button that calls the `dispatch` function, like so:
...
You can access these variables via your `countReducer`, where `state` is passed
as the first argument, and arguments to `dispatch` are passed as the latter:
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=01%3A%20useReducer%3A%20simple%20Counter&em=alexgrlee%40gmail.com).
